...
export async function fetchYoutubeTranscript(videoUrl, env = {}, chatId = null, userId = null, origin = null) {
  ...
  if (env.ASSEMBLYAI_API_KEY && chatId && userId && origin) {
    ...
    logger.warn('[YouTube orchestrator] Premium pipeline bermasalah. Mengalihkan ke scrapers native...');
  }
  ...
  try {
    ...
    if (captionTracks && captionTracks.length > 0) {
      const bestTrack = TranscriptParser.selectBestTrack(captionTracks);
      logger.info(`[YouTube orchestrator] Trek terbaik ditemukan di halaman embed: ${bestTrack.languageCode}`);
      return await TranscriptParser.downloadXmlTranscript(bestTrack.baseUrl, customProxy, logger);
    } else {
  ...
  throw new Error(`Seluruh jalur transkripsi video gagal dieksekusi. Detail kegagalan: ${errors.join(' | ')}`);
}